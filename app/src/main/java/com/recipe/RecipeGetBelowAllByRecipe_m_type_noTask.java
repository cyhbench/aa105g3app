...
public class RecipeGetBelowAllByRecipe_m_type_noTask extends AsyncTask<Object, Integer, List<RecipeVO>>{
    private final static String TAG = "RcpGetAllByRcpTypNOTask";
    private final static String ACTION = "getAllByRecipe_m_type_no";
    ...
    private String getRemoteOneData(String url, String jsonOut) throws IOException {
        ...
        bw.write(jsonOut);
        Log.d(TAG, "jsonOut(用中標籤編號得食譜): " + jsonOut);
        bw.close();
        ...
        if (responseCode == 200) {
            ...
            Log.d(TAG, "response code(用中標籤編號得食譜): " + responseCode);
        }
        connection.disconnect();
        Log.d(TAG, "jsonIn(用中標籤編號得食譜): " + jsonIn);
